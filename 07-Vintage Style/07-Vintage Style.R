...
lat1 <- 22.855; lat2 <- 22.997; lon1 <- 113.476; lon2 <- 113.59202508003357
# bbbox
...
sf_data <- st_read("07-Vintage Style/data/osm-2020-02-10-v3.11_china_guangzhou.mbtiles")
...
plot <- ggplot() +
  geom_sf(data = sf_data, fill = "#9EBFAA", color = NA, alpha = .8) +
  ...
  geom_sf(data = waterways2, color = "#9EBFAA", lwd = .5, alpha = .8) +
  ...
  theme_void() +
  theme(panel.background = element_rect(fill = alpha("#F5E4C3", 0.9), color = NA)) 
...
ggsave(filename = "07-Vintage Style/Outputs/map.png", plot = plot, height = 10, width = 7.5, units = "in", dpi = 300)
img <- image_read("07-Vintage Style/Outputs/map.png")
...
img_final <- image_annotate(img_final, 
                            ...
                            gravity = "south",
                            location = "+50+50")
image_write(img_final, path = "07-Vintage Style/Outputs/07-Luming-Vintage.png")
